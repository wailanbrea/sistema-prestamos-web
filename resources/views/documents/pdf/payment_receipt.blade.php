...
@php($currencyCode = $payment->loan->currency ?? currency())
</head>
<body>
    ...
    <table>
        <tr><th>Monto recibido</th><td class="right">{{ $currencyCode }} {{ number_format((float) $payment->amount, 2) }}</td></tr>
        <tr><th>Capital aplicado</th><td class="right">{{ $currencyCode }} {{ number_format((float) $payment->principal_paid, 2) }}</td></tr>
        <tr><th>Interés aplicado</th><td class="right">{{ $currencyCode }} {{ number_format((float) $payment->interest_paid, 2) }}</td></tr>
        <tr><th>Mora aplicada</th><td class="right">{{ $currencyCode }} {{ number_format((float) $payment->late_fee_paid, 2) }}</td></tr>
        <tr><th>Balance anterior</th><td class="right">{{ $currencyCode }} {{ number_format((float) $payment->previous_balance, 2) }}</td></tr>
        <tr><th>Balance nuevo</th><td class="right">{{ $currencyCode }} {{ number_format((float) $payment->new_balance, 2) }}</td></tr>
    </table>
